...
var ATTRIBUTES = {
    ...
    Conference: ["name","muted","beep","startConferenceOnEnter", "endConferenceOnExit", "waitUrl", "waitMethod", "maxParticipants"],
    Redirect:["method"],
    Reject:["reason"],
...
Element.prototype.addElement = function(elem){
  if (!this.data[elem.getName()]) this.data[elem.getName()] = [];
  return this.data[elem.getName()].push(elem.toJSON());
};
...
Response.prototype.addPause = function (attributes) {
    return this.add("Pause", undefined, attributes);
};  
...
Dial.prototype.addConference= function (body, attributes) {
    return this.add("Conference", body, attributes);
};
